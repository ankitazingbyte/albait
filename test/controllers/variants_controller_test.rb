...
class VariantsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create variant" do
    assert_difference('Variant.count') do
      post variants_url, params: { variant: { no: @variant.no, product_id: @variant.product_id, yes: @variant.yes } }
    end
  ...
  test "should update variant" do
    patch variant_url(@variant), params: { variant: { no: @variant.no, product_id: @variant.product_id, yes: @variant.yes } }
